...
export async function getSubscription(manager : Wetland.Scope, subscriptionId : number, user? : Entities.User) : Promise<Entities.Subscription>
{
    let options : any = { populate : [ 'subreddits', 'author', 'user'] };
    let sub : Entities.Subscription = await manager.getRepository(Entities.Subscription).findOne({ id: subscriptionId }, options );
...
export async function getNewSubscription( manager : Wetland.Scope,  user : Entities.User, authorName : string, ...subredditNames : string[]  ) : Promise<Entities.Subscription>
{
    throwIfIllegalCharacters(authorName);

    let sub : Entities.Subscription;
    let author : Entities.Author = await manager.getRepository(Entities.Author).findOne({ name_lower: authorName.toLowerCase() }, { });
    ...
    else
    {
        ...
        if (sub != null)
        {
            ...
            Log.I(`User already subscribed to author: ${authorName}`);
        }
    }
    ...
    if (subredditNames != null && subredditNames.length > 0)
        await addSubredditToSubscription(manager, sub, ...subredditNames);

    return sub;
}

export async function addSubredditToSubscription( manager : Wetland.Scope, sub : Entities.Subscription, ...subredditNames : string[] )
{
    ...
    subredditNames.forEach( ( name : string ) => 
    { 
...
export async function removeSubredditFromSubscription( manager : Wetland.Scope, sub : Entities.Subscription, subredditName : string )
{
    throwIfIllegalCharacters(subredditName);
    let subreddit : Entities.Subreddit = await manager.getRepository(Entities.Subreddit).findOne({ name_lower: subredditName.toLowerCase() }, { });
